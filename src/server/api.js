...
function verificaAcesso(param,dados){
  const acessoEspecifico = getAcessos();
  if(acessoEspecifico==null)
    return dados;
  return dados.filter((dado)=>acessoEspecifico[param].includes(dado));
}
function getData(){
  const json = {
    user:getUserInfo(),
...
function getFrequencia(ano, polo, programa, curso, turma, disciplina) {
  copiaAlunosParaFrequencia(ano, polo, programa, curso, turma, disciplina);
