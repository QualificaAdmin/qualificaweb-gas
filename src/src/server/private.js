...
function abrePlanilhaAlunos(ano, polo, programa, curso, turma) {
    const pasta = Diretorio.porCaminho(`/${ano}/${polo}/${programa}/${curso}/${turma}`);
    const arquivoPlanilha = getOrCreatePlanilha(pasta, "Alunos");
    const planilha = SpreadsheetApp.open(arquivoPlanilha);
    return planilha;
}
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "getAcessos": () => (/* binding */ getAcessos),
...
function getAcessos(parametro, valor) {
    return new _services_UsuarioServico__WEBPACK_IMPORTED_MODULE_0__.UsuarioServico().getAcesso(Session.getActiveUser().getEmail());
}
...
class GrupoUsuarioRepositorio extends _RepositorioBase__WEBPACK_IMPORTED_MODULE_2__.RepositorioBase {
    ...
    static map(obj) {
        const parse = {
            ...
            acesso: obj.fields.acesso
                ? Object.keys(obj.fields.acesso.mapValue.fields).reduce((acc, key) => {
                    const values = obj.fields.acesso.mapValue.fields[key].arrayValue.values;
                    acc[key] = values.map((value) => value.stringValue);
                    return acc;
                }, {})
                : null,
    ...
    get() {
        ...
        const grupos = data.map((grupo) => {
            let { nome, permissoes, ativo, acesso } = GrupoUsuarioRepositorio.map(grupo);
            return new _domain_base_GrupoUsuario__WEBPACK_IMPORTED_MODULE_0__.GrupoUsuario(nome, permissoes, ativo, acesso);
        });
        return grupos;
    }
    getById(id) {
        let data = this.contexto.getById(id);
        let { nome, permissoes, ativo, acesso } = GrupoUsuarioRepositorio.map(data[0]);
        const user = new _domain_base_GrupoUsuario__WEBPACK_IMPORTED_MODULE_0__.GrupoUsuario(nome, permissoes, ativo, acesso);
        return user;
    }
}
...
/* harmony export */ });
class GrupoUsuario {
    constructor(nome, permissoes, ativo, acesso = null) {
        this.nome = nome;
    ...
    temAcesso(parametro, valor) {
        return this.acesso.some((a) => a[parametro] == valor);
    }
}
...
__webpack_require__.g.getAcessos = _aplication__WEBPACK_IMPORTED_MODULE_1__.getAcessos;
